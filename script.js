...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        try {
            ...
            console.error('❌ Erreur init crédits:', error);
        } finally {
...
function updateAuthUI(user) {
    ...
    const profileBubbleName = document.getElementById('profile-bubble-name');

    if (user && !user.isAnonymous) {
        // Utilisateur Google connecté
        profileBox.style.display = 'flex';
        if (profileLinks) profileLinks.classList.add('is-visible');
        ...
        if (profileBubbleName) profileBubbleName.textContent = displayName;
        ...
        if (profileLinks) profileLinks.classList.remove('is-visible');
        if (profileBubbleName) profileBubbleName.textContent = '';
    }
}
